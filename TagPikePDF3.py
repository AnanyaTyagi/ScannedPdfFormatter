...
def page_add_invisible_mcid_stream(pdf, page_obj, mcid, text):
    """
    Inject a marked-content span with a *visible but fully transparent* text object.
    This lets Acrobat compute a highlight box when you click the tag.

    - Uses /GS1 with /ca 0 and /CA 0
    - Tr = 0 (paint text), but alpha = 0 keeps it invisible
    """
    ...
    # Ensure /Font dict and /F1 font exist
    fonts = resources.get('/Font', None)
    if fonts is None:
        fonts = Dictionary()
        resources['/Font'] = fonts
    ...
    # Ensure an ExtGState with 0 alpha exists as /GS1
    extg = resources.get('/ExtGState', None)
    if extg is None:
        extg = Dictionary()
    ...
    if '/GS1' not in extg:
        gs = Dictionary({
            '/Type': Name('/ExtGState'),
            '/ca': 0.0,   # non-stroking alpha
            '/CA': 0.0,   # stroking alpha
        })
        try:
            extg['/GS1'] = pdf.make_indirect(gs)
        except AttributeError:
            extg['/GS1'] = gs
    ...
    # NOTE: this places the transparent text at the text matrix origin (0,0).
    # If you want the highlight near the real content, you’d pass coordinates and
    # replace "0 0 Td" with e.g. "x y Td".
    content = f"""q
/Span << /MCID {int(mcid)} >> BDC
/GS1 gs
BT
...
""".encode('utf-8')
    ...
    contents = page_obj.get('/Contents', None)
    if contents is None:
        page_obj[Name('/Contents')] = pdf.make_indirect(new_stream)
    elif isinstance(contents, Array):
        contents.append(pdf.make_indirect(new_stream))
    else:
        page_obj[Name('/Contents')] = Array([contents, pdf.make_indirect(new_stream)])


def attach_text_to_elem(pdf, se, page_obj, text, *, parenttree, page_key, mcid):
    se[Name('/ActualText')] = as_str(text)
    se[Name('/T')] = as_str(text)  # helps Acrobat show the label
...
def main(pdf_in, fused_dir, struct_dir, pdf_out, title="Tagged Reconstruction", lang="en-US"):
    ...
    # ----- Write ParentTree and SAVE (this was missing) -----
