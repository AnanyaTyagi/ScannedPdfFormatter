...
def render_page_png(page, scale=2.0):
    """Render a PDF page to PNG bytes at scale (2.0 ~ 144dpi, 3.0 ~ 216dpi)."""
    mat = fitz.Matrix(scale, scale)
    pix = page.get_pixmap(matrix=mat, alpha=False)
    # Convert to PNG bytes
    return pix.tobytes("png"), pix.width, pix.height, scale

def collect_words(page):
...
def words_to_lines(words, y_tol=3.0):
    """Group words into lines by y proximity, then left-to-right inside each line."""
...
def facsimile_page_html(page, page_num, scale, img_b64, img_w, img_h):
    """
    Page background = rasterized image (looks identical).
    Overlay = absolutely positioned line-level <p> elements (taggable).
    """
    ...
    for ln in lines:
        ...
        # Heuristic font size from line height
        fsize = max(10, int((y1 - y0) * scale * 0.9))

        # Join words with spaces (use word order left->right)
        text = " ".join(w["text"] for w in ln).strip()
        if not text:
            continue

        line_ps.append(
            f"<p class='lt' style='left:{left}px;top:{top}px;font-size:{fsize}px;'>{escape_html(text)}</p>"
        )

    page_div = f"""
<section class="page" aria-label="Page {page_num}">
  <div class="bg" style="width:{img_w}px;height:{img_h}px;
       background-image:url(data:image/png;base64,{img_b64});"></div>
...
def build_facsimile_html(pdf_path, dpi_scale=2.0):
    doc = fitz.open(pdf_path)
    pages = []
    for pnum, page in enumerate(doc, start=1):
        png_bytes, w, h, scale = render_page_png(page, scale=dpi_scale)
        img_b64 = base64.b64encode(png_bytes).decode("ascii")
        pages.append(facsimile_page_html(page, pnum, scale, img_b64, w, h))

    css = """
    <style>
      /* Neutral page background so it looks like the original */
      html, body { margin:0; padding:0; background:#fff; }
      main[role="document"] { padding: 0; }  /* ✅ this CSS already expects role="document" */
      ...
      .bg { background-size: contain; background-repeat: no-repeat; background-position: top left; }

      .overlay { position: absolute; left: 0; top: 0; pointer-events: none; }

      /* Line text: nearly-transparent ink so renderers keep/tag it */
      .lt {
        position: absolute;
        margin: 0; padding: 0;
        white-space: nowrap; line-height: 1;
        font-family: Helvetica, Arial, sans-serif;
        color: rgba(0,0,0,0.01);  /* ✅ not fully transparent */
...
<html lang="en">
<meta charset="utf-8">
<title>Facsimile of {escape_html(pdf_path)}</title>
{css}
<body>
<main role="document" aria-label="Document">  <!-- ✅ Step 3: was role="main" -->
{''.join(pages)}
</main>
</body>
</html>"""
...
# -------------- CLI --------------
def process_pdf_facsimile(pdf_path: str, scale=2.0) -> str:
    return build_facsimile_html(pdf_path, dpi_scale=scale)

def main():
    if len(sys.argv) < 3:
        print("Usage: python format_pdf.py input.pdf out.html [--pdf out.pdf] [--engine prince|weasy] [--mode facsimile|structured]")
        sys.exit(1)
    pdf_path = sys.argv[1]
    out_html = sys.argv[2]
    out_pdf = None
    ...
    if mode == "facsimile":
        html = process_pdf_facsimile(pdf_path, scale=2.0)  # 2.0≈144dpi; use 3.0 for crisper text
    else:
        html = process_pdf_to_html(pdf_path)               # your existing structured pipeline

    write_html_and_optional_pdf(html, out_html, out_pdf, engine=engine)
